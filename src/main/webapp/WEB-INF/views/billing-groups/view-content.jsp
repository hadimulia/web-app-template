                ...
                <div class="card bg-light">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            ...
                            <c:choose>
                                <c:when test="${billingGroup.autoGenerate == 1}">
                        ...
                        <div class="mb-2">
                            ...
                            <small class="text-muted">
                                <fmt:formatDate value="${billingGroup.createdAt}" pattern="dd MMM yyyy HH:mm"/>
                            </small>
                        </div>
                        <div class="mb-2">
                            <strong>Last Updated:</strong><br>
                            <small class="text-muted">
                                <fmt:formatDate value="${billingGroup.updatedAt}" pattern="dd MMM yyyy HH:mm"/>
                            </small>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-bordered" id="customersTable">
                        ...
                        <tbody>
                            <c:forEach var="assignment" items="${customerAssignments}">
                                <tr>
                                    <td><strong>${assignment.customerCode}</strong></td>
                                    <td>${assignment.companyName}</td>
                                    <td>${assignment.contactPerson}</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${not empty assignment.customPrice}">
                                                <fmt:formatNumber value="${assignment.customPrice}" type="currency" currencyCode="${billingGroup.currency}"/>
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${assignment.discountPercent > 0}">
                                                <span class="text-success">${assignment.discountPercent}%</span>
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <td>
                                        <fmt:formatDate value="${assignment.startDate}" pattern="dd MMM yyyy"/>
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${not empty assignment.endDate}">
                                                <fmt:formatDate value="${assignment.endDate}" pattern="dd MMM yyyy"/>
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="${pageContext.request.contextPath}/customers/view/${assignment.customerId}" 
                                               class="btn btn-outline-info btn-sm" title="View Customer">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <form action="${pageContext.request.contextPath}/billing-groups/remove-customer/${assignment.assignmentId}" 
